...
import { effectiveProgramDate } from "@/lib/time";
...
export async function loadActionableTaskForUser(taskId: string, userId: string) {
  ...
  const { data: day } = await sb.from("program_days")
    .select("id, date, program_id, programs(id, timezone, start_date, end_date, day_unlock_offset_minutes)")
  ...
  const program = Array.isArray(programRaw) ? programRaw[0] : programRaw as {
    id: string; timezone: string; start_date: string; end_date: string; day_unlock_offset_minutes: number;
  };
  ...
  const today = effectiveProgramDate(program.timezone, program.day_unlock_offset_minutes ?? 0);
